...
let moviesTitle = movies.map((movieTitle) => {
...
let createListMovies = (movieTitle) => {
  createListMovies.innerHTML = '';
    movieTitle.forEach((titleName) => {
    const {title} = titleName;
    let movieTitle = document.createElement('li');
    container.appendChild(movieTitle);
    // moviesList.appendChild(moviesName);
    movieTitle.innerHTML = `The movie title is ${title}`
  });
}
// createListMovies();
...
let titleAndDescription = movies.map((titleDescription) => {
  return `The description for the movie ${titleDescription.title} is ${titleDescription.description} `;
})
// console.log(titleAndDescription);

let titleDescriptionGenre = movies.map ((moviesTiDeGe) => {
  return `The ${moviesTiDeGe.title} is a ${moviesTiDeGe.genre.name} movie and the description is: ${moviesTiDeGe.description}`;
})

// console.log(titleDescriptionGenre);

let titleDescriptionGenreDirector = movies.map ((moviesTiDeGeDi) => {
  return `The movie ${moviesTiDeGeDi.title} by ${moviesTiDeGeDi.director.name} is a ${moviesTiDeGeDi.genre.name} movie. The description is: ${moviesTiDeGeDi.description}`;
})

// console.log(titleDescriptionGenreDirector);

let sfMovies = movies.filter((movies) => {
  return movies.genre.name === 'Science Fiction'
})
//  console.log(sfMovies);

let adventureMovies = movies.filter((movies) => {
  return movies.genre.name === 'Adventure';
})

// console.log(adventureMovies);

let fantasyMovies = movies.filter((movies) => {
  return movies.genre.name === 'Fantasy';
})
// console.log(fantasyMovies);

let releaseDate = movies.filter((movie) => {
  return movie.release >= 2007;
})
// console.log(releaseDate);

let genreAndReleaseDate = movies.filter((movie) => {
  return movie.category ===  "release" && movie.release > 2008;
  // console.log(movie.release);
})

// console.log(genreAndReleaseDate);
...
let createMovie = (title, genre, release, description, director, imageurl) => {                               //-2
  ...
  // list.forEach((movie) => {                                 //-4
    ...
    let productGenre = document.createElement("h3");
    contentContainer.appendChild(productGenre);
    productGenre.innerHTML = genre.name;

    let productRelease = document.createElement("h2");
    contentContainer.appendChild(productRelease);
    productRelease.innerHTML = release;
    ...
    contentContainer.appendChild(productDirectore);
    productDirectore.innerHTML = director.name;

    let productImageUrl = document.createElement("img");
    contentContainer.appendChild(productImageUrl);  
    // productImageUrl.innerHTML = imageurl;
